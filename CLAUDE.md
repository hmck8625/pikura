...
- **エージェント定義**: `.claude/agents/` ディレクトリにMarkdownファイルで定義
- **定義済みエージェント**:
  - `quality-checker` — 記事の品質チェック（E-E-A-T、SEO要件、コンテンツ品質、内部リンク）
  - `fact-checker` — 記事のファクトチェック（統計、ルール、商品情報、人名、施設、日付の検証）
  - `tournament-director` — 大会運営ディレクター「河野」（ピクラカップ企画・運営計画・コスト試算・リスク管理）
- **コンテンツパイプライン**: 執筆 → 品質チェック → ファクトチェック → 人間承認 → 公開
- **公開基準**: 著者情報・更新日あり、内部リンク3本以上、文字数2,000字以上、ファクトチェック済み

## 運用ルール

- **作業は日報に記載する**（新規作成 or 追記）。日報は `week{N}/` ディレクトリに `YYYY-MM-DD_日報.md` の命名規則で保存
- **幹部MTG議事録**は `week{N}/` に保存。命名規則: `YYYY-MM-DD_幹部MTG_{テーマ}.md`
- **記事公開ペース**: 毎日1〜2本（午前=リライト、午後=新規）。新規は品質チェック+ファクトチェック必須
